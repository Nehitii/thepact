...
-- J AVAIS ECRIT UNE EXPRESSION QUI FAIT SEMBLANT DE DERIVER :
--
--   greatest(1, (select count(*) from (...) x where false) + 3)
--
-- La sous-requete porte « where false », ne rend donc jamais de ligne,
-- et le compte vaut zero : l expression valait 3 en dur, habillee de
-- SQL mort. C est pire qu une constante — une constante se voit et se
-- corrige, celle-la se cache.
--
-- Les categories cosmetiques sont trois, et elles sont TROIS TABLES :
-- cosmetic_frames, cosmetic_banners, cosmetic_titles. Le nombre est
-- donc une propriete du schema, pas une donnee. On l ecrit en clair,
-- avec la raison a cote, plutot que de le maquiller.
...
comment on function public.categories_cosmetiques() is
  'Le nombre de familles de cosmetiques. Trois tables, trois familles : cadres, bannieres, titres.';
