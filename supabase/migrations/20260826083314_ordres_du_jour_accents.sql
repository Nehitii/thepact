...
-- Les libelles etaient ecrits sans accents — « Termine une etape »,
-- « Conscience ecrite », « Note une pensee ». Dans une application en
-- francais, ca se lit comme du texte casse. Rien d'autre ne change.
...
-- Les ordres deja poses aujourd'hui portent encore le texte sans
-- accents : on les reecrit plutot que d'attendre demain.
update public.daily_quests
   set title = 'Conscience écrite',
       description = 'Note une pensée dans le Chronolog.'
 where kind = 'journal_entry' and title = 'Conscience ecrite';

update public.daily_quests
   set description = 'Termine une étape d''une de tes missions.'
 where kind = 'complete_steps' and description = 'Termine une etape d''une de tes missions.';
