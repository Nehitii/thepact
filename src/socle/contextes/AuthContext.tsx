...
import * as Sentry from "@/socle/outils/sentry";
import { User, Session } from "@supabase/supabase-js";
import { supabase } from "@/socle/supabase/client";
import { useNavigate } from "react-router-dom";
/* LE SOCLE NE NOMME AUCUN DOMAINE.
 *
 * Ce fichier appelait `trackLogin` et `initializeAchievementTracking`
 * du domaine succes. Quatorze domaines importent ce contexte : lui
 * faire connaitre un domaine, c est le faire cesser d etre le socle —
 * et la garde l a dit des que `src/socle/` a existe.
 *
 * Il ANNONCE desormais la connexion, et `App.tsx` — qui a le droit de
 * tout connaitre — ecoute et compte. Meme idiome que OUVRIR_MIA et
 * SHORTCUT_HELP_EVENT ailleurs dans l application. */
export const CONNEXION_ETABLIE = "vowpact:connexion-etablie";
        ...
        if (event === "SIGNED_IN" && session?.user) {
          setTimeout(() => {
            window.dispatchEvent(
              new CustomEvent(CONNEXION_ETABLIE, { detail: { userId: session.user.id } }),
            );
          }, 0);
        }
