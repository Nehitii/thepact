/* LES REGLES DE REFLEXE : ce que M.I.A. sait repondre sans le modele.
 *
 * Une table de trois cents lignes, sortie de `logique/reflexes.ts` qui
 * en faisait 438 et dont la fonction de recherche n en fait que 51.
 * Chaque regle porte ses declencheurs, sa reponse, et la facon dont
 * elle lit l etat du jour.
 *
 * Elle vient avec ses aides de formulation — les nombres a la
 * francaise, les dates en clair, le choix d une variante sans repeter
 * la precedente. Rien ici ne connait React.
 */
import { gestesConnus, reflexesConnus as _r } from "@/domaines/mia/logique/possibles";
import type { EtatDuJour } from "@/domaines/mia/types";
import type { Reflexe } from "@/domaines/mia/logique/reflexes";

/* Le dernier texte servi par intention : on ne répète pas. */
export const dernier = new Map<string, string>();

export function choisir(intention: string, variantes: string[]): string {
...
export const enFrancais = (iso: string | null): string =>
  iso ? new Date(iso).toLocaleDateString("fr-FR", { day: "numeric", month: "long" }) : "sans échéance";

export const GESTES_CONNUS = gestesConnus();
