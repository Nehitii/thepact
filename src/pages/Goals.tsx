...
export default function Goals() {
  ...
  useEffect(() => {
    ...
    const loadGoals = async () => {
      ...
      const {
        data: goalsData
      } = await supabase.from("goals").select("*").eq("pact_id", pactData.id).order("created_at", {
        ascending: false
      });
      if (goalsData) {
        // Fetch actual step counts for each goal
        const goalsWithStepCounts = await Promise.all(goalsData.map(async goal => {
          const {
            data: steps
          } = await supabase.from("steps").select("status").eq("goal_id", goal.id);
  ...
  const activeGoals = goals.filter(g => g.status === 'not_started' || g.status === 'in_progress' || g.status === 'validated');
  ...
  return <div className="min-h-screen pb-20 relative" style={{
    background: '#00050B'
  }}>
      ...
      <div className="relative z-10">
        ...
        {/* Goals Tabs */}
        {goals.length === 0 ? <Card className="border-2 border-primary/30 bg-[#00050B]/90 backdrop-blur-xl shadow-[0_0_30px_rgba(91,180,255,0.15)]">
          ...
          </Card> : <Tabs value={activeTab} onValueChange={v => setActiveTab(v as "active" | "completed")} className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-[#00050B]/80 border border-primary/20 p-1">
              <TabsTrigger value="active" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_15px_rgba(91,180,255,0.3)] font-rajdhani tracking-wide">
                Active ({activeGoals.length})
              </TabsTrigger>
              <TabsTrigger value="completed" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_15px_rgba(91,180,255,0.3)] font-rajdhani tracking-wide">
                Completed ({completedGoals.length})
              </TabsTrigger>
            </TabsList>

            <TabsContent value="active" className="space-y-6 mt-8">
              {activeGoals.length === 0 ? <Card className="border-2 border-primary/30 bg-[#00050B]/90 backdrop-blur-xl shadow-[0_0_30px_rgba(91,180,255,0.15)]">
                  <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                    <div className="h-20 w-20 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center mb-6 shadow-[0_0_20px_rgba(91,180,255,0.2)]">
                      <Plus className="h-10 w-10 text-primary" />
                    </div>
              ...
              return <div key={goal.id} className="relative group">
                      ...
                      <div className="absolute -inset-1 rounded-lg opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500" style={{
                  background: `${difficultyColor}20`
                }} />
                      
                      <Card className="relative overflow-hidden cursor-pointer transition-all duration-300 hover:scale-[1.02] border-2 border-primary/30 bg-[#00050B]/95 backdrop-blur-xl shadow-[0_0_20px_rgba(91,180,255,0.1)]" onClick={() => navigate(`/goals/${goal.id}`)}>
                        ...
                        <CardContent className="p-6 relative z-10">
                          <div className="flex gap-5">
                            ...
                            <div className="relative flex-shrink-0">
                              {goal.image_url ? <div className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-primary/40" style={{
                          ...
                          boxShadow: `0 0 20px ${difficultyColor}50, inset 0 0 20px ${difficultyColor}15`
                                  ...
                                  <Trophy className="h-10 w-10 relative z-10" style={{
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-4 mb-4">
                                <div className="flex-1 min-w-0">
                                   <h3 className="font-bold text-xl leading-tight mb-2 line-clamp-2 font-orbitron tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-[#E7F8FF] via-[#BEEBFF] to-[#E7F8FF]" style={{
                              ...
                              filter: 'drop-shadow(0 0 8px rgba(190, 235, 255, 0.4))'
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <Badge variant="outline" className="text-xs font-bold font-rajdhani uppercase tracking-wider" style={{
                          ...
                          <div className="flex items-center justify-between mt-5 pt-4 border-t border-primary/20">
                            <div className="flex items-center gap-3">
                              {goal.potential_score && <div className="flex items-center gap-2 font-rajdhani font-bold">
                                  <Sparkles className="h-5 w-5 text-yellow-400" style={{
                            filter: 'drop-shadow(0 0 6px rgba(250, 204, 21, 0.8))'
                          }} />
                                  <span className="text-yellow-400 text-base" style={{
            ...
            <TabsContent value="completed" className="space-y-6 mt-8">
              {completedGoals.length === 0 ? <Card className="border-2 border-primary/30 bg-[#00050B]/90 backdrop-blur-xl shadow-[0_0_30px_rgba(91,180,255,0.15)]">
                  <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                ...
                background: 'linear-gradient(135deg, hsl(var(--muted)) 0%, hsl(var(--muted) / 0.8) 100%)'
                      ...
                      <CardContent className="p-5 relative">
                        <div className="flex gap-4">
                          {/* Left Section: Image + Focus Star */}
                          <div className="relative flex-shrink-0">
                            {goal.image_url ? <div className="relative w-24 h-24 rounded-lg border-2 shadow-lg grayscale" style={{
                        borderColor: `${difficultyColor}66`
                      }}>
                                <img src={goal.image_url} alt={goal.name} className="w-full h-full object-cover rounded-lg" />
                        ...
                        background: `linear-gradient(135deg, ${difficultyColor}20, ${difficultyColor}10)`
                          ...
                          <div className="flex-1 min-w-0 space-y-3">
                            {/* Title & Badges */}
                            <div>
                              <h3 className="text-lg font-bold mb-2 line-clamp-2 font-orbitron tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-[#E7F8FF] via-[#BEEBFF] to-[#E7F8FF]" style={{
                          textShadow: '0 0 12px rgba(190, 235, 255, 0.25), 0 1px 2px rgba(0, 0, 0, 0.3)',
                          filter: 'drop-shadow(0 0 8px rgba(190, 235, 255, 0.4))'
                        }}>
                                {goal.name}
                              ...
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <Badge variant="outline" className="text-xs font-semibold capitalize px-2.5 py-0.5 opacity-75" style={{
                            ...
                            <div className="space-y-1.5">
                              ...
                              <div className="h-2.5 bg-muted rounded-full overflow-hidden shadow-inner relative" style={{
                          border: `2px solid ${difficultyColor}66`
                        }}>
                                <div className="h-full transition-all duration-500 rounded-full bg-green-500 relative" style={{
                            width: "100%"
                          }}>
                            ...
                            {/* XP Points Earned */}
                            {goal.potential_score > 0 && <div className="flex items-center gap-1.5 text-xs">
