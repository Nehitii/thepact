...
export function RanksCard({ userId }: RanksCardProps) {
  ...
  const handleAddRank = () => {
    const newRank: Rank = {
      id: "new",
      min_points: ranks.length > 0 ? (ranks[ranks.length - 1].min_points + 500) : 0,
      name: "",
      frame_color: "#5bb4ff",
  ...
  const handleSaveRank = async (rank: Rank) => {
    if (!rank.name.trim()) {
      toast({ title: t("profile.pact.ranks.validationError"), description: t("profile.pact.ranks.nameRequired"), variant: "destructive" });
      return;
    }

    if (isNewRank) {
      const { error } = await supabase.from("ranks").insert({
        user_id: userId, min_points: rank.min_points, max_points: rank.max_points || null,
        name: rank.name.trim(), logo_url: rank.logo_url, background_url: rank.background_url,
        background_opacity: rank.background_opacity, frame_color: rank.frame_color,
        glow_color: rank.glow_color, quote: rank.quote,
      });
      if (error) { toast({ title: t("common.error"), description: error.message, variant: "destructive" }); return; }
      toast({ title: t("profile.pact.ranks.createdTitle"), description: t("profile.pact.ranks.createdDesc", { name: rank.name }) });
    } else {
      const { error } = await supabase.from("ranks").update({
        min_points: rank.min_points, max_points: rank.max_points || null,
        name: rank.name.trim(), logo_url: rank.logo_url, background_url: rank.background_url,
        background_opacity: rank.background_opacity, frame_color: rank.frame_color,
        glow_color: rank.glow_color, quote: rank.quote,
      }).eq("id", rank.id);
      if (error) { toast({ title: t("common.error"), description: error.message, variant: "destructive" }); return; }
      toast({ title: t("profile.pact.ranks.updatedTitle"), description: t("profile.pact.ranks.updatedDesc", { name: rank.name }) });
    }
    queryClient.invalidateQueries({ queryKey: ["rank-xp"] });
  };

  const handleDeleteRank = async (rank: Rank) => {
    if (!confirm(t("profile.pact.ranks.deleteConfirm", { name: rank.name }))) return;
    const { error } = await supabase.from("ranks").delete().eq("id", rank.id);
    if (error) {
      toast({ title: t("common.error"), description: error.message, variant: "destructive" });
    } else {
      toast({ title: t("profile.pact.ranks.deletedTitle"), description: t("profile.pact.ranks.deletedDesc", { name: rank.name }) });
      queryClient.invalidateQueries({ queryKey: ["rank-xp"] });
    }
  };
  ...
  return (
    <PactSettingsCard
      ...
      title={t("profile.pact.ranks.title")}
      description={t("profile.pact.ranks.description")}
    >
        ...
        <div className="mb-4 p-4 rounded-xl bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/30">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-orbitron text-primary/70 uppercase tracking-wider">
              {t("profile.pact.ranks.currentRank")}
            </span>
          </div>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex justify-between text-[10px] font-rajdhani text-muted-foreground">
              <span className="flex items-center gap-1"><Target className="h-3 w-3" />{t("profile.pact.ranks.globalProgress")}</span>
          ...
          <div className="mt-3 p-2.5 rounded-lg bg-background/50 border border-primary/20">
            ...
            <p className="text-[9px] text-muted-foreground/60 mt-1">
              {t("profile.pact.ranks.maxXPDesc")}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="relative">
        {ranks.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground font-rajdhani border border-dashed border-primary/20 rounded-lg bg-primary/5">
            ...
            <p>{t("profile.pact.ranks.empty")}</p>
            <p className="text-xs text-muted-foreground/70 mt-1">{t("profile.pact.ranks.emptyHint")}</p>
          </div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-orbitron font-semibold text-sm uppercase tracking-wide truncate"
                          style={{ color: rank.frame_color || '#5bb4ff', textShadow: `0 0 8px ${rank.glow_color || 'rgba(91,180,255,0.3)'}` }}>
                          {rank.name}
                        </div>
                        <div className="text-xs text-muted-foreground font-rajdhani">
                          {rank.min_points.toLocaleString()}+ XP
                          {isCurrentRank && <span className="ml-2 text-primary/70">({t("profile.pact.ranks.current")})</span>}
                        </div>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="ghost" onClick={() => handleEditRank(rank)} className="h-8 w-8 text-primary/70 hover:text-primary hover:bg-primary/20">
                          <Edit2 className="h-3.5 w-3.5" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => handleDeleteRank(rank)} className="h-8 w-8 text-destructive/70 hover:text-destructive hover:bg-destructive/20">
      ...
      <Button onClick={handleAddRank} variant="outline"
        ...
        {t("profile.pact.ranks.addButton")}
      </Button>
