  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between" style={{ padding: "14px 24px", background: "rgba(255,140,0,0.05)", borderBottom: "1px solid rgba(255,140,0,0.12)" }}>
        ...
        <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 9, letterSpacing: 2, padding: "3px 10px", border: "1px solid rgba(255,140,0,0.3)", color: "#ff8c00", background: "rgba(255,140,0,0.06)", clipPath: "polygon(6px 0%, 100% 0%, calc(100% - 6px) 100%, 0% 100%)" }}>
          STANDBY
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="flex gap-4 flex-col md:flex-row">
              ...
              <div className="w-full md:w-[200px] flex flex-col gap-3">
                ...
                <div className="rounded-sm border border-[var(--nexus-separator)] bg-[var(--nexus-inner-bg)] p-3">
                  <span className="text-[8px] font-orbitron uppercase tracking-[0.15em] text-[var(--nexus-text-dimmer)] block mb-2">STATISTIQUES</span>
                ...
                <div className="rounded-sm border border-[var(--nexus-separator)] bg-[var(--nexus-inner-bg)] p-3">
                  <span className="text-[8px] font-orbitron uppercase tracking-[0.15em] text-[var(--nexus-text-dimmer)] block mb-2">FILTRE DIFF.</span>
                  <div className="grid grid-cols-3 gap-1.5">
                    {DIFF_FILTERS.map((f) => (
                      <button key={f.key} onClick={() => setDiffFilter(diffFilter === f.key ? null : f.key)}
                        className={cn("text-[7px] font-orbitron font-bold uppercase tracking-wider py-1 rounded-sm border transition-all", diffFilter === f.key ? "border-current bg-current/10" : "border-[var(--nexus-separator)] hover:border-current/30")}
                        style={{ color: f.color }}
                      >{f.label}</button>
                    ))}
                    <button onClick={() => setDiffFilter(null)} className="text-[7px] font-orbitron uppercase tracking-wider py-1 rounded-sm border border-[var(--nexus-separator)] text-[var(--nexus-text-dimmer)] hover:text-[var(--nexus-text-label)] transition-all">
        ...
        {viewState === "spinning" && tempWinner && (
          <motion.div key="spinning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 flex flex-col items-center gap-4">
            <div className="text-[10px] font-mono text-primary animate-pulse tracking-[0.3em] uppercase">Running Algorithm...</div>
