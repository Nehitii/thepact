...
export function ActiveMissionCard({ mission, onAbandon, onComplete, className }: ActiveMissionCardProps) {
  ...
  const handleAbandon = async () => { setIsAbandoning(true); await onAbandon(); setIsAbandoning(false); };
  const handleComplete = async () => { setIsCompleting(true); await onComplete(); setIsCompleting(false); };
  const goToGoal = () => { navigate(`/goals/${mission.goal_id}`); };
  ...
  return (
    ...
    >
      {urgencyLevel === 'critical' && (
        <motion.div className="absolute inset-0 border-2 border-destructive/60" style={{ borderRadius: "4px" }} animate={{ opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 1, repeat: Infinity }} />
      )}
      ...
      <div className={cn(
        ...
        urgencyLevel === 'normal' && "border-[var(--nexus-separator)] bg-[var(--nexus-hover-bg)]"
      )}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="ds-t-label font-orbitron uppercase tracking-[0.15em] text-foreground/70">
              Active Mission
            </span>
          </div>
          <div className={cn(
            "flex items-center gap-1.5 px-2 py-1 ds-t-label font-mono font-bold",
            ...
            urgencyLevel === 'normal' && !isExpired && "bg-primary/10 text-primary"
          )} style={{ borderRadius: "4px" }}>
      ...
      <div className="relative z-10 p-4 space-y-4">
        <div className="flex items-start gap-3">
          <div className={cn(
            ...
            urgencyLevel === 'normal' && "bg-primary/10 border border-primary/20"
          )} style={{ borderRadius: "4px" }}>
            <Target className={cn("w-5 h-5", urgencyLevel === 'critical' && "text-destructive", urgencyLevel === 'warning' && "text-amber-400", urgencyLevel === 'normal' && "text-primary")} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs text-muted-foreground font-rajdhani uppercase tracking-wider">Goal</p>
            <h4 className="text-sm font-orbitron font-bold text-foreground truncate">{mission.goal_name}</h4>
          </div>
        </div>

        <div className="p-3 bg-[var(--nexus-inner-bg)] border border-[var(--nexus-separator)]" style={{ borderRadius: "4px" }}>
          <p className="ds-t-label text-muted-foreground font-rajdhani uppercase tracking-wider mb-1">Focused Step</p>
        ...
        {isExpired && (
          <div className="flex items-center gap-2 p-2 bg-destructive/20 border border-destructive/30" style={{ borderRadius: "4px" }}>
            ...
            <span className="text-xs text-destructive font-rajdhani">Deadline passed! Complete or abandon to continue.</span>
          </div>
        )}

        <div className="grid grid-cols-2 gap-2">
          <Button onClick={handleComplete} disabled={isCompleting || isAbandoning} className="font-rajdhani text-xs bg-gradient-to-r from-health/80 to-health hover:from-health hover:to-health/80 border border-health/30" style={{ borderRadius: "4px" }}>
            <Check className="w-4 h-4 mr-1" />
            {isCompleting ? 'Saving...' : 'Complete'}
          </Button>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="outline" disabled={isCompleting || isAbandoning} className="font-rajdhani text-xs border-destructive/30 text-destructive hover:bg-destructive/10" style={{ borderRadius: "4px" }}>
            ...
            <AlertDialogContent className="bg-card/95 backdrop-blur-xl border-destructive/30">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="font-rajdhani">Are you sure you want to abandon this mission? This action cannot be undone. You will be able to pick a new mission after abandoning.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel className="font-rajdhani">Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={handleAbandon} className="bg-destructive hover:bg-destructive/80 font-rajdhani">{isAbandoning ? 'Abandoning...' : 'Yes, Abandon'}</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>

        <button onClick={goToGoal} className="w-full flex items-center justify-center gap-1 ds-t-label text-muted-foreground hover:text-primary transition-colors font-rajdhani uppercase tracking-wider py-1">
