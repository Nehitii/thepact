...
export function CountdownPanel({ projectStartDate, projectEndDate, goalsCompleted, totalGoals, pactName = "OPERATION ASCENSION" }: CountdownPanelProps) {
  ...
  return (
    <div
      ...
      style={{
        borderRadius: 4,
        border: "1px solid var(--nexus-countdown-border)",
    ...
    >
      ...
      <div className="absolute top-0 bottom-0 left-0" style={{ width: 3, background: "linear-gradient(180deg, rgba(255,23,68,0.8), rgba(255,23,68,0.1))", boxShadow: "0 0 12px rgba(255,23,68,0.6)" }} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] items-center" style={{ minHeight: 100 }}>
        ...
        <div
          ...
          style={{ padding: "20px 28px", borderColor: "rgba(255,23,68,0.15)", background: "rgba(255,23,68,0.04)" }}
        >
          ...
          <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 8, letterSpacing: 3, color: "rgba(255,23,68,0.6)", textTransform: "uppercase" as const, textAlign: "center", animation: "blink 2s step-end infinite" }}>
            ALERTE<br />CRITIQUE
          </div>
        </div>
        ...
        <div style={{ padding: "20px 28px" }}>
          <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 8, letterSpacing: 4, color: "rgba(255,23,68,0.4)", textTransform: "uppercase" as const, marginBottom: 12 }}>
          ...
          <div className="flex items-center">
            {[
              ...
              <div key={t.label} className="flex items-center">
                ...
                {i < 3 && (
                  <span style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "clamp(22px, 3vw, 36px)", fontWeight: 700, color: "rgba(255,23,68,0.3)", margin: "0 6px 12px", animation: "colonBlink 1s step-end infinite" }}>
                    :
                  </span>
                )}
              </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div className="flex justify-between" style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 8, letterSpacing: 2, color: "rgba(255,23,68,0.35)", marginBottom: 5 }}>
              <span>DÉBUT · {startStr}</span>
              <span>{Math.round(calc.progressPct)}% ÉCOULÉ</span>
              <span>FIN · {endStr}</span>
            </div>
            <div className="relative overflow-hidden" style={{ height: 6, background: "rgba(255,23,68,0.06)", border: "1px solid rgba(255,23,68,0.12)", borderRadius: 1 }}>
              <div className="relative" style={{ height: "100%", width: `${calc.progressPct}%`, background: "linear-gradient(90deg, rgba(255,100,0,0.7), rgba(255,23,68,0.9))", boxShadow: "0 0 8px rgba(255,23,68,0.5)", borderRadius: 1 }}>
                <div className="absolute top-0 left-0 right-0" style={{ height: "50%", background: "rgba(255,255,255,0.08)" }} />
                <div className="absolute top-0 right-0" style={{ width: 20, height: "100%", background: "linear-gradient(90deg, transparent, rgba(255,100,100,0.6))", animation: "barPulse 2s ease-in-out infinite" }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{ padding: "20px 28px", borderColor: "rgba(255,23,68,0.12)", background: "rgba(255,23,68,0.02)", minWidth: 180 }}
        >
          {[
            ...
            <div key={row.key} className="flex flex-col gap-0.5">
              <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 7, letterSpacing: 3, color: "rgba(255,23,68,0.35)", textTransform: "uppercase" as const }}>
                {row.key}
              </span>
              <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: row.critical ? 12 : 11, letterSpacing: 1, color: row.critical ? "#ff1744" : "rgba(255,140,140,0.7)", textShadow: row.critical ? "0 0 6px rgba(255,23,68,0.5)" : "none" }}>
                {row.val}
              </span>
            </div>
          ))}
