  ...
  return (
    <div className={cn("w-full max-w-2xl mx-auto", className)}>
      <div className="relative p-2 rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-between gap-3 shadow-2xl">
        ...
        {/* MODULE LINKS */}
        <div className="flex-1 grid grid-cols-3 gap-2" role="navigation" aria-label="Quick module actions">
          {actions.map((action) =>
            action.owned ? (
              <motion.button
                key={action.id}
              ...
              >
                ...
                <div className="flex flex-col items-center">
                  <span className="text-[10px] font-bold font-orbitron tracking-wide text-foreground">
                ...
                <div className="absolute top-2 right-2 w-1 h-1 rounded-full bg-white/20 group-hover:bg-white/60" aria-hidden="true" />
              </motion.button>
            ) : (
              <div
                key={action.id}
...
function NewGoalContent() {
  return (
    <>
      <div className="absolute inset-0 bg-cyan-400/20 blur-xl opacity-0 group-hover:opacity-50 transition-opacity" aria-hidden="true" />
