...
import { PrismTicker } from "./PrismTicker";
...
interface PrismHeroProps {
  ...
  sessionId: string;
  /** 0..1 — share of modules with data */
  density: number;
  /** count of modules currently producing live signals */
  liveModules: number;
  totalModules?: number;
}
...
export function PrismHero({
  ...
  const SEGMENTS = 8;
  const filled = Math.round(density * SEGMENTS);
  ...
  return (
    <motion.div
      ...
      className="prism-panel prism-panel--primary relative pl-7 pr-5 py-4 mb-6 overflow-hidden isolate"
    >
      ...
      {/* Lateral neon accent with breath */}
      <div
        ...
        style={{
          ...
          animation: "prism-breath 3.6s ease-in-out infinite",
      ...
      <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start gap-3">
            ...
            <h1
              className={cn(
                "font-orbitron text-xl md:text-2xl font-bold tracking-tight min-w-0",
                TONE_COLOR[insight.tone],
              )}
            >
              <PrismTicker key={`${insight.title}-${period}`} text={insight.title} />
            </h1>
          </div>
            ...
            <span className="hidden md:inline-flex items-center gap-1.5">
              ...
              <span className="inline-flex gap-[2px]">
                {Array.from({ length: SEGMENTS }).map((_, i) => (
                  <span
                    key={i}
                    className={cn("prism-density-seg", i < filled && "is-on")}
                    style={{ animationDelay: `${i * 90}ms` }}
                  />
                ))}
              </span>
              <span className="tabular-nums prism-text-cyan">
                <PrismAnimatedNumber
                  ...
                  return (
                    <span
                      ...
                      style={{
                        background: on
                          ? "hsl(var(--prism-cyan))"
                          : "hsl(var(--prism-cyan) / 0.18)",
                        boxShadow: on ? "0 0 6px hsl(var(--prism-cyan) / 0.7)" : undefined,
                      }}
                    />
                  );
                })}
