...
export function OrbitDistribution({ items, size = 280 }: OrbitDistributionProps) {
  const center = size / 2;
  const maxCount = Math.max(...(items.length ? items.map((i) => i.count) : [1]), 1);
  ...
  if (items.length === 0) return <OrbitDistributionEmpty />;

  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
        <circle cx={center} cy={center} r={6} fill="hsl(var(--prism-cyan))" opacity={0.85} />
        <circle cx={center} cy={center} r={14} fill="none" stroke="hsl(var(--prism-cyan) / 0.4)" strokeWidth={0.5} />

        {orbits.map((o, idx) => (
          <g key={o.label}>
            <circle cx={center} cy={center} r={o.radius} fill="none" stroke={o.color}
              strokeOpacity={0.18} strokeWidth={0.5} strokeDasharray="2 4" />
            <g
              style={{
            ...
            >
              {o.dots.map((d, i) => (
                <circle key={i} cx={d.x} cy={d.y} r={2.2} fill={o.color}
                  opacity={o.intensity} style={{ filter: `drop-shadow(0 0 4px ${o.color})` }} />
              ))}
            </g>
          </g>
        ))}
...
function OrbitDistributionEmpty() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-3">
      <div className="relative h-24 w-24">
        <div
          className="absolute inset-0 rounded-full border border-dashed border-[hsl(var(--prism-cyan)/0.4)]"
