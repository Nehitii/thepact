...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      {children}
      {/* CORRECTION ICI : Ajout de 'z-50' au début des classes */}
      <DialogPrimitive.Close className="absolute z-50 right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity data-[state=open]:bg-accent data-[state=open]:text-muted-foreground hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
