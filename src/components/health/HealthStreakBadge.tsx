...
export function HealthStreakBadge({ size = "md", showLabel = true, className }: HealthStreakBadgeProps) {
  ...
  const tierColors = {
    ...
    uncommon: "text-hud-phosphor",
    rare: "text-blue-500",
    epic: "text-purple-500",
    legendary: "text-hud-amber",
  };
  
  const tierGlows = {
    common: "",
    uncommon: "drop-shadow-[0_0_8px_hsl(var(--hud-phosphor)/0.5)]",
    rare: "drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]",
    epic: "drop-shadow-[0_0_12px_rgba(147,51,234,0.6)]",
    legendary: "drop-shadow-[0_0_16px_hsl(var(--hud-amber)/0.7)]",
  };
  ...
  return (
    <motion.div className={cn("flex items-center gap-2", className)}
      initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200 }}>
      <div className="relative">
        <motion.div
          animate={currentStreak > 0 ? { scale: [1, 1.1, 1] } : {}}
          transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>
          <Flame className={cn(iconSizes[size], tierColors[tier], tierGlows[tier], currentStreak === 0 && "opacity-30")} />
        </motion.div>
        {longestStreak > 0 && currentStreak === longestStreak && currentStreak >= 7 && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1 -right-1">
            <Trophy className="w-3 h-3 text-hud-amber" />
          </motion.div>
        )}
      </div>
      <div className="flex flex-col">
        <span className={cn("font-bold font-orbitron leading-none", sizeClasses[size], tierColors[tier])}>
          {currentStreak}
        </span>
        {showLabel && (
