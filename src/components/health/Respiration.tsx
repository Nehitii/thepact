...
/* ═══════════════════════════════════════════════════════════════
   RESPIRER, ET LE FAIRE MAINTENANT
   ...
   de parler en public — alors que ce ne sont pas les memes besoins, et
   pas les memes rythmes.

   Il est desormais sur la page, ouvert, pret. On ne clique pas pour
   ouvrir un outil : on clique pour commencer.

   QUATRE RYTHMES, NOMMES PAR CE QU ON VIENT CHERCHER.

   « 4-7-8 » ne dit rien a personne ; « s endormir » se choisit sans
   reflechir. Le detail des temps reste affiche pour qui le veut, en
   petit, sous le nom.
   ═══════════════════════════════════════════════════════════════ */
...
/* Les rythmes sont des classiques, pas des inventions : coherence
   cardiaque (5-5), boite (4-4-4-4), 4-7-8 pour l endormissement, et un
   apaisement rapide ou l expiration domine. */
...
export function Respiration() {
  ...
  return (
    <section>
      <h2 className="hlt-titre">{t("health.breath.title", "Respirer")}</h2>

      <div className="hlt-souffle">
        <div className="hlt-rond">
          ...
          <span className="hlt-rond-texte">
            ...
            <span>{enCours ? reste : "—"}</span>
          </span>
        </div>

        <div>
          <h3 className="hlt-souffle-nom">{t(`health.breath.pattern.${schema.id}.nom`, schema.id)}</h3>
          <p className="hlt-souffle-quoi">{t(`health.breath.pattern.${schema.id}.quoi`, "")}</p>
          ...
          <div className="hlt-schemas" role="group" aria-label={t("health.breath.choose", "Choisir un rythme")}>
            {SCHEMAS.map((s) => (
              ...
              >
                ...
                {" · "}
                {s.temps.filter((n) => n > 0).join("-")}
              </button>
            ))}
          </div>

          <div className="hlt-souffle-gestes">
            <button type="button" className="hlt-bouton" onClick={() => setEnCours((v) => !v)}>
