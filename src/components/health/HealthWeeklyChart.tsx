  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
      <HUDFrame className="p-6" scanLine>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-4 text-[10px] text-muted-foreground font-mono uppercase tracking-wider">
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 bg-blue-500" /> {t("health.metrics.sleep")}
            </span>
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 bg-hud-phosphor" /> {t("health.metrics.activity")}
            </span>
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 bg-hud-amber" /> {t("health.metrics.stress")}
            </span>
          </div>
        </div>

        <div className="flex gap-4 relative">
          {/* Target zone band (y: 3-5 = 60%-100%) */}
            ...
            return (
              <div key={day} className={cn("flex-1 text-center relative z-10", isToday && "relative")}>
                ...
                <div className="flex gap-1 justify-center h-32 items-end mb-2">
                  <motion.div className="w-3 bg-blue-500/90"
                    initial={{ height: 0 }}
                    animate={{ height: `${getBarHeight(data?.sleep_quality)}%` }}
                    transition={{ delay: 0.1 * i, duration: 0.5 }}
                  />
                  <motion.div className="w-3 bg-hud-phosphor/90"
                    initial={{ height: 0 }}
                    animate={{ height: `${getBarHeight(data?.activity_level)}%` }}
                    transition={{ delay: 0.1 * i + 0.05, duration: 0.5 }}
                  />
                  <motion.div className="w-3 bg-hud-amber/90"
                    initial={{ height: 0 }}
        ...
        {weeklyData.length === 0 && (
          <div className="text-center text-muted-foreground/70 text-sm mt-4 font-mono">
